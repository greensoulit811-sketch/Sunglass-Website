...
export default function LandingPageView() {
  ...
  const { formatCurrency, settings, t } = useSiteSettings();
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (requiresTrxId && !transactionId.trim()) {
      toast.error(t('checkout.transactionIdRequired'));
      return;
    }
    ...
    try {
      await createOrder.mutateAsync({
        order: {
          ...
          payment_method_name: selectedPayment?.name || t('checkout.cashOnDelivery'),
              ...
              {selectedProduct && (
                <div className="bg-white p-10 lg:p-12 border border-gray-100 shadow-[0_50px_100px_-30px_rgba(0,0,0,0.1)] space-y-10 relative overflow-hidden group">
                   ...
                   <div className="relative space-y-6 pt-12 border-t border-gray-100">
                      <div className="flex justify-between text-[11px] font-bold uppercase tracking-[0.4em] text-gray-400"><span>{t('cart.subtotal')}</span><span className="text-black">{formatCurrency(subtotal)}</span></div>
                      <div className="flex justify-between text-[11px] font-bold uppercase tracking-[0.4em] text-gray-400"><span>{t('cart.shipping')}</span><span className="text-black">{formatCurrency(shippingCost)}</span></div>
                      <div className="flex justify-between items-center pt-10 border-t-2 border-red-500">
                         <span className="text-2xl lg:text-3xl font-serif font-black uppercase italic tracking-tighter">{t('cart.total')}</span>
              ...
              <form onSubmit={handleSubmit} className="space-y-12">
                 ...
                 <div className="space-y-8">
                    <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
                       ...
                       <h3 className="text-xl font-bold font-serif italic tracking-tight">{t('checkout.contactInfo')}</h3>
                    </div>
                    <div className="grid grid-cols-1 gap-6">
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-500">{t('checkout.fullName')} *</label>
                          <Input name="fullName" value={formData.fullName} onChange={handleChange} required className="h-12 bg-gray-50 border-gray-100 focus:bg-white focus:border-black transition-all" placeholder={t('checkout.fullName')} />
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-gray-500">{t('checkout.phone')} *</label>
                             <Input name="phone" value={formData.phone} onChange={handleChange} required className="h-12 bg-gray-50 border-gray-100 focus:bg-white focus:border-black transition-all" placeholder={t('checkout.phone')} />
                          </div>
                          <div className="space-y-2">
                             <label className="text-xs font-bold text-gray-500">{t('checkout.emailOptional')}</label>
                             <Input name="email" type="email" className="h-12 bg-gray-50 border-gray-100 focus:bg-white focus:border-black transition-all" placeholder={t('checkout.email')} />
                          </div>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="space-y-8">
                    <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
                       <div className="w-2 h-6 bg-black"></div>
                       <h3 className="text-xl font-bold font-serif italic tracking-tight">{t('checkout.shippingAddress')}</h3>
                    </div>
                    <div className="space-y-6">
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-500">{t('checkout.city')} *</label>
                          <Input name="city" value={formData.city} onChange={handleChange} required className="h-12 bg-gray-50 border-gray-100 focus:bg-white focus:border-black transition-all" placeholder={t('checkout.city')} />
                       </div>
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-500">{t('checkout.address')} *</label>
                          <Input name="address" value={formData.address} onChange={handleChange} required className="h-12 bg-gray-50 border-gray-100 focus:bg-white focus:border-black transition-all" placeholder={t('checkout.addressPlaceholder')} />
                       </div>
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-500">{t('checkout.orderNotes')}</label>
                          <textarea className="w-full min-h-[100px] p-4 bg-gray-50 border border-gray-100 focus:bg-white focus:border-black transition-all resize-none outline-none text-sm" placeholder={t('checkout.orderNotesPlaceholder')}></textarea>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-6">
                        <div className="flex items-center gap-3 border-b border-gray-100 pb-3">
                           <h3 className="text-sm font-bold tracking-tight">{t('checkout.shippingMethod')}</h3>
                        </div>
                          ...
                          {paymentMethods.map(pm => (
                            <div key={pm.id} className="space-y-3">
                               ...
                               {formData.paymentMethodId === pm.id && requiresTrxId && (
                                 <Input value={transactionId} onChange={e => setTransactionId(e.target.value)} placeholder={t('checkout.enterTransactionId')} className="h-12 border-2 border-red-500 text-xs font-bold" />
                               )}
                            </div>
                          ))}
                       </div>
